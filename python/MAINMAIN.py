...
tokens = ["STRING", "RBRACE", "LBRACE", "COMMA", "LPAREN", "RPAREN", "QUOTE", "SEMICOLON", "EQUALS", "AND",
          "OR", "COLON", "MINUS", "ASSIGN", "DATE", "TIME", "TIME_DURATION", "LSQRBRACE", "RSQRBRACE"] + list(
    reserved.values())
...
def func_code_run(funcName, expr):
    ...
    if expr != None:
        match expr[0]:
            case 'UNAV':
                run_unavailability(expr[1])
            case 'AV':
                run_availability(expr[1])
            case _:
    ...
    try:
        for el in parm:
            if el[0] == 'type' and el[1] != 'services':
                showError("Query type \"" + el[2] + "\" is not supported")
                return False
            elif el[0] == 'period':
...
test_string2 = """
                    set service:
                        ...
                        duration: 2h,
                        status: true,
                        description: "check level of oil in the engine and all controllers",
                        priority: high;
                """
test_string3 = """
                 query:
...
test_string4 = """
                    employee: "Harry Potter";
                    set scheduled_service:
                        service: "Engine check",
                        client: "Arya Stark",
                        status: scheduled,
                        material: "oil cleaning fluid",
                        date: 2025-05-16,
                        description: "check level of oil in the engine and all controllers";
                """
test_string5 = """  employee: "Harry Potter";
                    batch:
                        set scheduled_service:
                            employee: "Luke Skywalker",
                            service: "Engine check",
...
# check_employee_availability(22,0)
